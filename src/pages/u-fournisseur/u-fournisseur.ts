...
@Component({
  selector: 'page-u-fournisseur',
  templateUrl: 'u-fournisseur.html',
})
export class UFournisseurPage {
  ...
  constructor( public toastCtrl: ToastController,public afdb: AngularFireDatabase,private calendar:Calendar ,private plt:Platform ,public navCtrl: NavController, public navParams: NavParams,public sanitizer : DomSanitizer) {
    ...
    this.articleRef = firebase.database().ref('/fournisseurs' );
...
this.adresse="" ;
  ...
  this.afdb.object("fournisseurs/"+this.item.key).update({
  nom : this.nom ,
    prenom : this.prenom ,
    phone : this.phone ,
    cin : this.cin,
