...
@Component({
  selector: 'page-a-stock',
  templateUrl: 'a-stock.html',
})
export class AStockPage {
  Image1 : string ;
...
Add() {
  ...
  for (var j of this.facture) {
    if(j.article==charticle){
      j.prix=(Number(j.quantite)+Number(this.quantite))*Number(p) ;
      j.quantite=Number(j.quantite)+Number(this.quantite) ;
      return false ;
    }
    
  }
    this.facture.push({article : charticle, fournisseur :chreservation,quantite :Number(this.quantite) , prix :Number(p)*Number(this.quantite)  }) ;
